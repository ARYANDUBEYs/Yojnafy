...
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), "person_a_matcher"))
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), "person_b_chat"))
...
def speak(text: str, language_code: str = "hi-IN"):
    audio_path = matcher_app.text_to_speech(text, language_code)
...
def listen(file: UploadFile = File(...), language_code: str = "hi-IN"):
    ...
    transcript = matcher_app.speech_to_text(temp_path, language_code)
